...
- SQL-миграции хранятся в `migrations/*.sql`.
- При старте приложения они применяются автоматически один раз (таблица `schema_migrations`).
- Для production это значит: деплоите код — миграции применятся на первом HTTP-запросе.
